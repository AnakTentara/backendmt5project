...
input int    Inp_SD_LookbackBars    = 25;    // S/D swing lookback bars
input double Inp_SD_RetestZone_Pts  = 80.0;  // Retest buffer zone (points)
input double Inp_MinSL_Points       = 200.0; // Initial SL distance (points)
input double Inp_LockProfit_Pts     = 400.0; // Trigger distance to lock SL+ (points)
input double Inp_LockedProfit_Value = 50.0;  // Profit locked at SL+ (points)
input double Inp_TrailingDist_Pts   = 400.0; // Dynamic trailing distance for big wave (points)
input double Inp_TP1_RR             = 2.0;   // TP1 Risk:Reward
input double Inp_TP2_RR             = 5.0;   // TP2 Risk:Reward
input double Inp_TP3_TrailingMult   = 2.0;   // TP3 trailing distance multiplier
input double Inp_MaxSL_Points       = 350.0; // Maximum SL distance (points)
input int    Inp_LocalSwingBars     = 5;     // Local M5 bars for swing
input double Inp_SL_Buffer_Pips     = 10.0;  // SL buffer beyond structure (points)
input bool   Inp_UseTPDraft         = true;  // Enable draft TP mode
...
void EvaluateEntries() {
    ...
    ulong ticket = OpenPositionDraftMode(entry_price, lots, sl_price, draft);
    if(ticket > 0) {
